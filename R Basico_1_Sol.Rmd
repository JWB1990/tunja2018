...
```{r}
colibri_vector<-c("Coeligena wilsoni", "Coeligena prunellei", "Coeligena torquata", "Coeligena phalerata",
                  "Coeligena bonapartei", "Coeligena helianthea", "Coeligena lutetiae", "Coeligena violifer", "Coeligena iris")
as.numeric(colibri_vector)
```
...
```{r}
colibri_masa[c(T, T, T, T, T, F, T, T, T, T)]
```
...
```{r}
colibri_masa_rd<-round(colibri_masa)
length(which(colibri_masa_rd==7))
```
...
Usando colcetes con tablas (matrices o data.frames) requiere dos argumentos porque son basicamente vectores en dos dimensiones. Con la funcion `dim()` puedes ver las dimensiones de un objeto. 
```{r}
dim(c_df)
```
...
```{r}
c_df[c_df$especie_caracter %in% c("Coeligena prunellei", "Coeligena torquata"),]
```